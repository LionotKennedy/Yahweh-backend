...
class SectionMenuImage {
  static async getBySection(sectionName) {
    const {
      ...
      "SELECT * FROM section_menus WHERE section = $1",
      [sectionName]
  ...
  static async update(sectionName, src) {
    const {
      ...
      `INSERT INTO section_menus (section, src)
       VALUES ($1, $2)
       ON CONFLICT (section) 
       DO UPDATE SET src = $2
       RETURNING *`,
  ...
  static async create({
    ...
    const {
      ...
      "INSERT INTO section_menus (section, src) VALUES ($1, $2) RETURNING *",
      [section, src]
  ...
  static async getAll() {
    const {
      rows
    } = await pool.query("SELECT * FROM section_menus");
    return rows;
  }
